...
const chipVariants = cva(
  `group/chip inline-flex shrink-0 items-center justify-center gap-1.5 rounded-full border text-sm font-medium whitespace-nowrap bg-clip-padding transition-all duration-short ease-standard outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 ${uiDisabledInteractive} aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 [&_svg]:pointer-events-none [&_svg]:shrink-0`,
  {
    variants: {
      variant: {
        outline:
          "border-border bg-background hover:bg-accent hover:text-accent-foreground aria-pressed:border-foreground/15 aria-pressed:bg-accent aria-pressed:text-accent-foreground",
        subtle:
          "border-transparent bg-muted text-muted-foreground hover:bg-accent hover:text-accent-foreground aria-pressed:bg-accent aria-pressed:text-foreground",
        default:
          "border-transparent bg-background text-foreground hover:bg-accent hover:text-accent-foreground aria-pressed:bg-inverse aria-pressed:text-inverse-foreground aria-pressed:hover:bg-inverse/80 active:aria-pressed:translate-y-px",
      },
      size: {
...
function Chip({
  ...
  const withCheck = showCheck ?? (variant !== "default" && !onRemove)
  ...
  const removablePressedShell =
    variant === "default"
      ? "has-[[aria-pressed=true]]:bg-inverse has-[[aria-pressed=true]]:text-inverse-foreground"
      : "has-[[aria-pressed=true]]:bg-accent has-[[aria-pressed=true]]:text-accent-foreground"
