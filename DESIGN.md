...
- **Grayscale**: `--grayscale-5` ~ `--grayscale-150` (16단계, 쿨 블루 틴트) · 흰 표면은 `--white`
...
| `formFieldGapTight` | Input ↔ Helper (필드 **안**) | **8 (확정)** |
...
**Disabled · no-surface 규칙:** `ghost` · `link` · Toggle default · SidebarMenuButton처럼 rest에 채움·윤곽이 없는 표현은 disabled 시 `bg-disabled`/`border-disabled` 면을 **추가하지 않는다** (`uiDisabledNoSurface`). **`outline` variant** disabled는 rest 윤곽 색은 그대로 두고, disabled 시 **border `{disabled-border}`** + bg transparent + `{disabled-foreground}` (`uiDisabledOutline`). 솔리드(default·secondary)는 `uiDisabledFilledInteractive`. 필드·Select 등 rest 윤곽 컨트롤은 `uiDisabledInteractive`.
...
| 프로젝트 | 경로 | 연결 방식 |
|---------|------|----------|
| 리노벨 스튜디오 | `/Users/user/Desktop/프로젝트/upnunde-test/app` | `github:upnunde/Renovel-Studio-DS#v0.1.54` |
...
**불변식:** 세 검증 모두 `0-DRIFT` / `무결성 OK` 여야 한다 = JSON 정본과 웹 `src/tokens.css` 값이 완전 일치(커스텀·SD 파이프라인 양쪽), 스펙의 모든 `{ref}`가 실재 토큰. 웹 산출물이 안 바뀌므로 **기존 소비자(리노벨, v0.1.54 pin) 영향 0**. `src/tokens.css` 는 이제 **"JSON 정본의 웹 산출물"** 성격이며, 값 변경은 JSON에서 하고 verify로 확인한다.
